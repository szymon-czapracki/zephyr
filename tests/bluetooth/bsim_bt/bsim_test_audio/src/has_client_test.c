...
static volatile uint8_t g_active_preset_index;
...
static void active_preset_cb(struct bt_has *has, int err, uint8_t index)
{
	if (err != 0) {
			      ...
			      struct bt_has_preset *preset)
{
	...
	printk("index %d properties 0x%02x name %s\n", preset->index, preset->properties,
	       preset->name);
...
static void test_main(void)
{
	...
	if (err < 0) {
		FAIL("Failed to get active preset index (err %d)\n", err);
		return;
	}

	WAIT_FOR(g_active_preset_notified);

	printk("Got active preset index %d\n", g_active_preset_index);

	index_prev = g_active_preset_index;
	...
	index_curr = g_active_preset_index;
	...
	err = bt_has_preset_name_set(g_has, index_curr, "Custom");
	if (err < 0) {
