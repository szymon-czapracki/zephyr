...
static void has_discover_cb(struct bt_conn *conn, struct bt_has *has, uint8_t type)
{
	if (has == NULL) {
...
static void has_active_preset_cb(struct bt_has *has, int err, uint8_t index)
{
	if (err != 0) {
		...
		shell_print(ctx_shell, "Preset (%d) set successful", index);
	}
}
			  ...
			  const char *name)
{
	if (err != 0) {
		...
		shell_print(ctx_shell, "Preset changed Index: %d Properties: 0x%02x Name: %s",
