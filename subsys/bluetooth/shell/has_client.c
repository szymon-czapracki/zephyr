...
static void has_preset_name_cb(struct bt_has *has, int err, uint8_t index, uint8_t properties,
			       const char *name)
{
	if (err != 0) {
...
static int cmd_has_change_preset_name(const struct shell *sh, size_t argc, char **argv)
{
	int result;
	int index = strtol(argv[1], NULL, 0);
	char *name = argv[2];
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	has_client_cmds,
	SHELL_CMD_ARG(init, NULL, "Initialize HAS client", cmd_has_client_init, 1, 0),
	SHELL_CMD_ARG(discover, NULL, "Discover HAS for current connection", cmd_has_discover, 1,
		      0),
	...
	SHELL_CMD_ARG(set_active_preset_prev, NULL, "Set previous active HAS preset",
		      cmd_has_active_preset_set_prev, 1, 0),
	SHELL_CMD_ARG(read_has_presets, NULL, "Read HAS presets", cmd_has_read_presets, 1, 0),
	SHELL_CMD_ARG(change_preset_name, NULL, "Change HAS preset name",
		      cmd_has_change_preset_name, 3, 0),
	SHELL_SUBCMD_SET_END);

SHELL_CMD_ARG_REGISTER(has_client, &has_client_cmds, "Bluetooth HAS shell commands", cmd_has_client,
		       1, 1);
