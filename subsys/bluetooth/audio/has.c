...
#define BT_HAS_CP_TX_MAX_LEN                                                                       \
	(1 + 2 /* Preset Changed */ + 3 /* Generic Update */ + BT_HAS_PRESET_NAME_MAX)
...
static struct has_client {
	...
	struct {
		...
		struct has_preset *preset_pending;
...
NET_BUF_SIMPLE_DEFINE_STATIC(cp_tx_buf, BT_HAS_CP_TX_MAX_LEN);

static uint8_t client_index(struct has_client *client)
{
	ptrdiff_t index = client - has_client_list;

	__ASSERT(index >= 0 && index < ARRAY_SIZE(has_client_list), "Invalid client pointer");
...
static void client_free(struct has_client *client)
{
	...
	for (int i = 0; i < ARRAY_SIZE(has_preset_list); i++) {
...
static struct has_client *client_find(struct bt_conn *conn)
{
	for (int i = 0; i < ARRAY_SIZE(has_client_list); i++) {
...
static inline int preset_changed_popcount(struct has_client *client)
{
...
static bool is_preset_changed_pending(struct has_client *client)
{
...
static uint8_t preset_index(struct has_preset *preset)
{
	ptrdiff_t index = preset - has_preset_list;
...
static bool is_read_preset_rsp_pending(struct has_client *client)
{
	return client->read_preset_rsp.preset_pending != NULL &&
	       client->read_preset_rsp.num_presets > 0;
}

static void control_point_tx_work_submit(struct has_client *client, k_timeout_t delay)
{
...
static int control_point_tx(struct has_client *client, struct net_buf_simple *buf)
{
...
static inline int preset_changed_index(struct has_client *client)
{
	int index = 0;
#if CONFIG_BT_HAS_PRESET_CNT > 32
	int i;
...
#endif
	__ASSERT(index >= 0 && index < ARRAY_SIZE(has_preset_list), "Invalid client pointer");
				     ...
				     bool is_last)
{
	...
	net_buf_simple_reset(&cp_tx_buf);

	BT_DBG("client %p preset %p is_last %d", client, preset, is_last);

	hdr = net_buf_simple_add(&cp_tx_buf, sizeof(*hdr));
	hdr->op = BT_HAS_OP_READ_PRESET_RSP;
	rsp = net_buf_simple_add(&cp_tx_buf, sizeof(*rsp));
	rsp->is_last = is_last ? 0x01 : 0x00;
	rsp->idx = preset->id;
	rsp->properties = preset->properties;
	net_buf_simple_add_mem(&cp_tx_buf, preset->name, slen);

	return control_point_tx(client, &cp_tx_buf);
}
				    ...
				    bool is_last)
{
	...
	pc = net_buf_simple_add(&cp_tx_buf, sizeof(*pc));
	...
	case BT_HAS_CHANGE_ID_GENERIC_UPDATE: {
		...
		gu = net_buf_simple_add(&cp_tx_buf, sizeof(*gu));
		gu->prev_idx = 0x01; /* TODO */
		gu->idx = preset->id;
		gu->properties = preset->properties;
		net_buf_simple_add_mem(&cp_tx_buf, preset->name, name_len);
	} break;
	case BT_HAS_CHANGE_ID_PRESET_DELETED:
	case BT_HAS_CHANGE_ID_PRESET_AVAILABLE:
	case BT_HAS_CHANGE_ID_PRESET_UNAVAILABLE:
		net_buf_simple_add_u8(&cp_tx_buf, preset->id);
		break;
	default:
		return -EINVAL;
	}
...
static inline void preset_changed_clear(struct has_client *client, uint8_t index)
{
	atomic_clear_bit(client->preset_changed.pending, index);
}

static inline void preset_changed_clear_all(struct has_client *client)
{
...
static void process_control_point_tx_work(struct k_work *work)
{
	struct has_client *client = CONTAINER_OF(work, struct has_client, cp_tx_work);
	...
	if (!client->conn || client->conn->state != BT_CONN_CONNECTED) {
		...
		preset = &has_preset_list[preset_changed_index(client)];
...
static int handle_read_preset_req(struct has_client *client, struct net_buf_simple *buf)
{
	...
	control_point_tx_work_submit(client, BT_HAS_CP_WORK_TIMEOUT);
...
static int handle_write_preset_name(struct has_client *client, struct net_buf_simple *buf)
{
	...
	return preset_name_set(req->idx, req->name, buf->len);
}
#else
	return -EOPNOTSUPP;
}
...
static int handle_set_active_preset(struct has_client *client, struct net_buf_simple *buf,
				    bool sync)
{
...
static int handle_set_next_preset(struct has_client *client, struct net_buf_simple *buf, bool sync)
{
...
static int handle_set_prev_preset(struct has_client *client, struct net_buf_simple *buf, bool sync)
{
	struct has_preset *preset = NULL;
...
static void connected(struct bt_conn *conn, uint8_t err)
{
	 ...
	 */
	for (int index = 0; index < ARRAY_SIZE(has_preset_list); index++) {
...
static int has_init(const struct device *dev)
{
	...
	BUILD_ASSERT(BT_HAS_CP_TX_MAX_LEN < CONFIG_BT_L2CAP_TX_MTU);
	...
	*out = &has_local;
	...
	for (int i = 0; i < ARRAY_SIZE(has_preset_list); i++) {
		...
		for (int j = 0; j < ARRAY_SIZE(param->preset_param); j++) {
			...
			if ((!preset_param || tmp->idx < preset_param->idx) &&
			    tmp->idx > last_preset_id) {
				preset_param = tmp;
			}
		}
...
int bt_has_preset_active_set(struct bt_has *has, uint8_t id)
{
	...
	if (BT_HAS_IS_LOCAL(has)) {
		...
		if (id != BT_HAS_PRESET_INDEX_NONE) {
			struct has_preset *preset = preset_get(id);
...
int bt_has_preset_read(struct bt_has *has, uint8_t id)
{
	if (IS_ENABLED(CONFIG_BT_HAS_CLIENT) && !BT_HAS_IS_LOCAL(has)) {
		CHECKIF(has == NULL)
		{
		...
		return bt_has_client_preset_read(has, id);
	}

	return -EOPNOTSUPP;
}

int bt_has_preset_read_multiple(struct bt_has *has, uint8_t start_idx, uint8_t count)
{
	if (IS_ENABLED(CONFIG_BT_HAS_CLIENT) && !BT_HAS_IS_LOCAL(has)) {
		CHECKIF(has == NULL)
		{
		...
		return bt_has_client_preset_read_multiple(has, start_idx, count);
	}
...
int bt_has_preset_availability_set(struct bt_has *has, uint8_t id, bool available)
{
	...
	if (BT_HAS_IS_LOCAL(has)) {
		...
		change_id = available ? BT_HAS_CHANGE_ID_PRESET_AVAILABLE :
		...
		if (((preset->properties & BT_HAS_PROP_AVAILABLE) > 0) != available) {
			...
			/* Do not send preset changed notification if the preset is hidden */
			if (!preset->hidden) {
				preset_changed(preset, change_id);
			}
		}
