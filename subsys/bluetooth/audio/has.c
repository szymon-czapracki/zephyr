...
int bt_has_register(const struct bt_has_register_param *param, struct bt_has **out)
{
	...
	*out = &has_local;
	 ...
	 * If the HA is part of a Binaural Hearing Aid Set, the HA shall set at least one of the
	 * Front Left bit or the Front Right bit to a value of 0b1 in the Sink Audio Locations
	 * characteristic value as defined in [2].
	 *
	 * A Banded Hearing Aid in the HA role shall set the Front Left and the Front Right bits to
	 * a value of 0b1 in the Sink Audio Locations characteristic value.
	 */
	...
	} else if (IS_ENABLED(CONFIG_BT_HAS_HEARING_AID_BINAURAL_RIGHT)) {
